...
_selectToastType(ArchiveHandle *AH, const char *toasttype)
{
	...
	/*
	 * XXX: this quoting business is obviously incorrect..  Fix later.
	 */
	if (strcmp(want, "\"bigint\"") == 0 ||
		strcmp(want, "bigint") == 0)
	{
		/* Special cases to avoid double quotes around the type names */
		appendPQExpBuffer(cmd, "SET default_toast_type = int8;");
	}
	else if (strcmp(want, "\"oid\"") == 0 ||
			 strcmp(want, "oid") == 0)
		appendPQExpBuffer(cmd, "SET default_toast_type = oid;");
	else
		Assert(false);
