...
check_toasted_attribute(HeapCheckContext *ctx, ToastedAttribute *ta)
{
	 ...
	 * under scrutiny here.  The two must agree.  Whether the pointer carries
	 * a compression method ID byte is irrelevant to the chunk_id type, so
	 * compare with that distinction (the low bit of the tag) masked out.
	 */
