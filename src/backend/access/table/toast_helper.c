								   ...
								   bool for_compression, bool check_main)
{
	...
	/*
	 * Define the lower-bound, depending on the TOAST table type used for
	 * values from the relation cache.
	 *
	 * Note: if we begin supporting multiple external TOAST pointer types in a
	 * single attribute value type, we should reconsider how this lower-bound
	 * is defined.  Should it be a minimum or a maximum value of all the TOAST
	 * pointer sizes supported for this relation?
	 */
