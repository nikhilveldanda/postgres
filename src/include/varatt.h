 ...
 * fixed part; see the description of va_compressed in varattrib_4b and the
 * VARTAG_ONDISK_*_LONG TOAST pointers below.
 */
 ...
 * which VARTAG_IS_ONDISK() and VARTAG_IS_ONDISK_LONG() rely on.
 */
...
{
	return ((tag & ~1) == VARTAG_ONDISK_OID || (tag & ~1) == VARTAG_ONDISK_OID8);
}
 ...
 *
 * The compressed-in-line format has a long form, used when the method bits
 * of va_tcinfo hold VARLENA_COMPRESS_METHOD_LONG: va_data[0] is then the
 * compression method ID, and the compressed data starts at va_data[1] (see
 * VARHDRSZ_COMPRESSED_LONG and VARCMID_4B_C).  This is deliberately not
 * described by a union member of its own, because that would increase
 * sizeof(varattrib_4b), and code all over the place inspects varlena headers
 * of unknown or smaller size through pointers of this type.
 */
typedef union
{
	...
	struct						/* Compressed-in-line format */
	{
		...
		char		va_data[FLEXIBLE_ARRAY_MEMBER]; /* Compressed data, or
													 * method ID + data */
	}			va_compressed;
} varattrib_4b;
...
/* method ID byte of the long form of the compressed-in-line format */
#define VARCMID_4B_C(PTR)	(((varattrib_4b *) (PTR))->va_compressed.va_data[0])
...
#define VARHDRSZ_COMPRESSED_LONG	(VARHDRSZ_COMPRESSED + sizeof(uint8))
